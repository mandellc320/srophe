        ...
        return 
            ...
            return 
                if($facet-value != '') then 
                    ...
                    else if($facet/facet:group-by[@function="facet:group-by-array"] or 
                    $facet/facet:group-by[@function="facet:collection"] or 
                    $facet/facet:group-by[@function="facet:taxonomy"] or
                    $facet/facet:group-by[@function="facet:group-by-array-poetess"]) then 
...
declare function facet:key($label, $value, $count, $facet-definition){
   ...
   let $new-fq := 
        ...
        else concat('fq=',encode-for-uri(normalize-space($facet-query)))
...
(:~ 
 ...
 : Constructs new URL for user action 'remove facet'
:)
declare function facet:selected-facets-display($results, $facet-definition){
    for $facet in tokenize($facet:fq,';fq-')
    let $facet-name := substring-before($facet,':')
    let $value := substring-after($facet,':')
    let $new-fq := string-join(
                    ...
                    return 
                        if($facet-param = $facet) then ()
                        else concat(';fq-',encode-for-uri($facet-param)),'')
    let $href := if($new-fq != '') then concat('?fq=',replace(replace($new-fq,';fq- ',''),';fq-;fq-',';fq-'),facet:url-params()) else ()
    return
        ...
        return 
                    <span class="label facet-label remove" title="Remove {$value}">
                        {concat($fn,': ', facet:controlled-labels($results, $value))} <a href="{$href}" class="facet icon"> x</a>
                    </span>
};
...
declare function facet:controlled-labels($results, $label) {
   let $taxonomies := $results//tei:taxonomy
   return 
    if(starts-with($label,'#')) then 
         let $id := replace($label,'#','')
         let $text := $taxonomies/descendant-or-self::*[@xml:id = $id][1]
         return normalize-space(string-join($text,' '))
    else $label
};
